...
function Calendar(whatMonth,whatYear) {
    ...
    for (i=1; i<=days[whatMonth]; i++, column++) {
      ...
      //±N271¦æµù¸Ñ«á, ¦A¨ú®ø¥H¤Uµù¸Ñ,«hµLªk¿ï¾Ü¤µ¤Ñ¤§«eªº¤é´Á
     
      if(whatMonth<todayDate.getMonth()){
    		output += getDayLink(i,true,whatMonth,whatYear);	
    	}
    	else if(i<todayDate.getDate() && whatMonth==todayDate.getMonth()){
    		output += getDayLink(i,true,whatMonth,whatYear);
    	}else{
    		output += getDayLink(i,false,whatMonth,whatYear);	
    	}
...
var names     = new makeArray0('¤@¤ë','¤G¤ë','¤T¤ë','¥|¤ë','¤­¤ë','¤»¤ë','¤C¤ë','¤K¤ë','¤E¤ë','¤Q¤ë','¤Q¤@¤ë','¤Q¤G¤ë');
